...
>Windows 10(x64), Python 3.7, OpenCV 4.1.0
...
* Compute the camera calibration matrix and distortion coefficients through a set of chessboard images.
* Apply a distortion correction to raw images.
* Combnine color information and gradient information to create a thresholded binary image.
...
#### If we want to get 3d information of lane from 2d image, we need to apply a perspective transform on 2d image. Camera calibraiton can help us get a good perspective transformation matrix. Also, the image distortion can be removed.

The code for camera calibration step is contained in the [`calibration.py`](calibration.py).  

The camera matrix(intrinsic parameters) and distortion coefficients calculated by the `cv2.calibrateCamera()` function with 20 9*6 sized [chessboard images](camera_cal). And applied this distortion correction to the test image using the `cv2.undistort()` function.  
...
Two approaches are applied to find lane lines.  
...
The code for Sliding window search is contained in the [`finding_lines.py`](finding_lines.py).  
